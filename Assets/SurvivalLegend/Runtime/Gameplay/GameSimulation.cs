...
namespace SurvivalLegend
{
    ...
    public sealed partial class GameSimulation
    {
        ...
        public GameSimulation(uint seed = 1, string specialD = "heal", string specialF = "haste", string character = "archer")
        {
        ...
        static Vector2 ClampWorld(Vector2 p) => new Vector2(Mathf.Clamp(p.x,24,1416), Mathf.Clamp(p.y,24,1416));
        ...
        public void MoveTo(Vector2 point) { if (!CombatActive) return; State.Order="move"; State.Destination=ClampWorld(point); State.TargetId=-1; State.AimSlot=-1; Effect(point,"move",.45f,12,"#b3f783"); }
        ...
        public void AttackMove(Vector2 point) { if (!CombatActive) return; State.Order="attack-move"; State.Destination=ClampWorld(point); State.TargetId=ClosestEnemy(point,AttackRange)?.Id ?? -1; State.AimSlot=-1; Effect(point,"move",.45f,12,"#ffb982"); }
        ...
        public bool Dodge(Vector2 cursor)
        {
            ...
            dodgeStart=p.Position; dodgeEnd=ClampWorld(p.Position+d.normalized*180);
            if((dodgeEnd-dodgeStart).sqrMagnitude<.000001f) return false;
            if(State.DodgeCharges==2) State.DodgeRecharge=10;
            State.DodgeCharges--; State.DodgeRemaining=.2f; PlayMotion(State.Character=="swordsman"?"roll":State.Character=="mage"?"phase":"lunge",Angle(d),Catalog.Config.DodgeDuration); Stop(); State.AimSlot=-1; return true;
        }
        void TickDodge(float dt)
        {
            if(State.DodgeCharges<2) { State.DodgeRecharge-=dt; while(State.DodgeRecharge<=.00001f && State.DodgeCharges<2) { State.DodgeCharges++; State.DodgeRecharge=State.DodgeCharges<2?State.DodgeRecharge+10:0; } }
            ...
            State.Player.Position=Vector2.Lerp(dodgeStart,dodgeEnd,1-State.DodgeRemaining/.2f);
        }
        public void Step(float delta)
        {
            float dt=Mathf.Clamp(delta,0,.05f); if(dt<=0) return;
            if(State.Phase==RunPhase.Dying) { State.PhaseElapsed+=dt; if(State.PhaseElapsed>=Catalog.Config.DeathSeconds) State.Phase=RunPhase.Results; return; }
            if(State.Phase==RunPhase.EliteDeath) { State.Projectiles.RemoveAll(s=>s.Hostile);State.Zones.RemoveAll(z=>z.Hostile); State.PhaseElapsed+=dt; if(State.PhaseElapsed>=Catalog.Config.EliteDeathSeconds) { State.SpecialReward=true; RollChoices(true); EnterPhase(RunPhase.AugmentEnter); } return; }
            if(State.Phase==RunPhase.AugmentEnter) { State.PhaseElapsed+=dt; if(State.PhaseElapsed>=Catalog.Config.CardDealSeconds) EnterPhase(RunPhase.Augment); return; }
            ...
            foreach(var s in State.Skills) s.Remaining=Mathf.Max(0,s.Remaining-dt*(1+s.Haste/Catalog.Config.HastePointsPerDoubleRate)*BuffModifier(b=>b.CooldownRate));
            ...
            if(!dodgingThisStep && attackTimer<=0 && target!=null && Vector2.Distance(p.Position,target.Position)<=AttackRange+target.Radius)
            {
            ...
            if(State.Phase==RunPhase.LevelUp) { State.PhaseElapsed+=dt; if(State.PhaseElapsed>=Catalog.Config.LevelUpSeconds) { EnterPhase(RunPhase.AugmentEnter); State.AimSlot=-1; } }
            else CheckLevelUp();
        }
        ...
        void UpdateMovement(float dt)
        {
            ...
            {
                var target=FindEnemy(State.TargetId);
                if(State.Order=="attack-move" && (target==null || Vector2.Distance(p.Position,target.Position)>480+target.Radius)) { target=ClosestEnemy(p.Position,Mathf.Max(480,AttackRange)); State.TargetId=target?.Id??-1; }
                if(target!=null) destination=Vector2.Distance(p.Position,target.Position)<=AttackRange+target.Radius ? (Vector2?)null : target.Position;
                else if(State.Order=="attack") { Stop(); return; }
            }
            if(!destination.HasValue) return;
            Vector2 d=destination.Value-p.Position; float travel=Mathf.Min(d.magnitude,p.Speed*BuffModifier(b=>b.MoveSpeed)*(p.HasteRemaining>0?Catalog.Specials["haste"].SpeedMultiplier:1)*dt);
            if(travel>.001f) { p.Facing=Angle(d); p.Moving=true; p.Position=ClampWorld(p.Position+d.normalized*travel); }
            if(d.magnitude<=travel+.001f && State.TargetId<0) Stop();
        }
        void BasicShot(EnemyState target,Vector2? aim=null)
        {
            ...
            var s=Shoot(origin,target.Position,State.Player.Damage*BuffModifier(b=>b.Damage),Catalog.Combat.PlayerShotSpeed,Catalog.Combat.PlayerShotRadius,Catalog.Combat.PlayerShotLifetime,false,false);
        ...
        void TickProjectiles(float dt)
        {
            foreach(var shot in State.Projectiles)
            {
                ...
                if(shot.TargetId>=0) { var target=FindEnemy(shot.TargetId); if(target==null){shot.Life=0;continue;} shot.Velocity=(target.Position-shot.Position).normalized*Mathf.Min(Catalog.Combat.PlayerShotSpeed,Vector2.Distance(shot.Position,target.Position)/dt); }
                if(shot.HasFlight)shot.FlightTraveled+=shot.Velocity.magnitude*dt;
                shot.Position+=shot.Velocity*dt; shot.Life-=dt;
                if(shot.Hostile) { if(SegmentDistance(State.Player.Position,before,shot.Position)<=shot.Radius+15) { DamagePlayer(shot.Damage,shot.SourceId); shot.Life=0; } }
            ...
            State.Projectiles.RemoveAll(s=>s.Life<=0 || s.Position.x< -30 || s.Position.y< -30 || s.Position.x>1470 || s.Position.y>1470);
        }
        ...
        void TickZones(float dt)
        {
            foreach(var z in State.Zones)
            {
                ...
                if(z.Hostile) { if(InZone(State.Player.Position,15,z)) DamagePlayer(z.Damage,z.SourceId); }
        ...
        public void DamagePlayer(float amount,int sourceId=-1)
        {
            ...
            float damage=MitigatedDamage(amount*BuffModifier(b=>b.IncomingDamage),p.Armor);
            ...
            State.DamageTaken+=Mathf.Min(p.Hp,damage); p.Hp=Mathf.Max(0,p.Hp-damage); p.Invulnerable=Catalog.Config.HitInvulnerability;
        ...
        public void DamageEnemy(EnemyState enemy,float amount,bool charge=true)
        {
            ...
            if(charge) State.Ultimate=Mathf.Min(Catalog.Config.UltimateMaximum,State.Ultimate+Catalog.Config.UltimateChargePerHit);
            if(enemy.Kind=="elite") amount*=1+State.Player.EliteDamage;
            ...
            if(enemy.Kind=="elite") { State.EliteKills++; deferredNormal=State.Phase==RunPhase.LevelUp?new List<AugmentChoice>(State.AugmentChoices):null; EnterPhase(RunPhase.EliteDeath); Stop(); State.AimSlot=-1; /* Removed after iteration; frozen hostile visuals cannot damage. */ }
        }
        void Effect(Vector2 position,string kind,float life,float radius,string color,string text=null,bool critical=false) { State.Effects.Add(new EffectState {Position=position,Kind=kind,Life=life,MaxLife=life,Radius=radius,Color=color,Text=text,Critical=critical}); }
    }
}
